...
void HelloWorldAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int band = 0; band < EQState::numBands; ++band)
    {
        bool muted = eqState.bandMutes[band];

        if (eqState.anySoloActive())
            muted |= !eqState.bandSolos[band];

        float gain = muted ? 0.0f : eqState.bandGains[band];
...
void HelloWorldAudioProcessor::setBandGain (int band, float gain)
{
    jassert (band >= 0 && band < EQState::numBands);
    eqState.bandGains[band] = gain;
}

void HelloWorldAudioProcessor::setBandMute (int band, bool mute)
{
    jassert (band >= 0 && band < EQState::numBands);
    eqState.bandMutes[band] = mute;
}

void HelloWorldAudioProcessor::setBandSolo (int band, bool solo)
{
    jassert (band >= 0 && band < EQState::numBands);
    eqState.bandSolos[band] = solo;
}
