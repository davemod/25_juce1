  ...
  void processBlock(juce::AudioBuffer<float> &buffer)
  {
    ...
    for (int ch = 0; ch < numCh; ++ch)
    {
      ...
      for (int n = 0; n < numSamples; ++n)
      {
        ...
        const float out = bandGains[0] * b1 + bandGains[1] * b2 +
                          bandGains[2] * b3 + bandGains[3] * b4;
...
private:
  ...
  void updateFilters()
  {
    ...
    // docs: designFIRLowpassWindowMethod creates linear-phase FIR coeffs
    // [oai_citation:0‡JUCE
    // Docs](https://docs.juce.com/master/structjuce_1_1dsp_1_1FilterDesign.html?utm_source=chatgpt.com)
    ...
    jassert(lp1Coeffs != nullptr && lp2Coeffs != nullptr &&
            lp3Coeffs != nullptr);
  ...
  size_t firOrder =
      127; // FIR order (=> 128 taps); bigger = steeper, more latency
